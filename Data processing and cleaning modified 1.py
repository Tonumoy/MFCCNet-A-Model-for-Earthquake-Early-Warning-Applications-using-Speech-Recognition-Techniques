...
def plot_fbank(fbank):
    ...
    for x in range(2):
        for y in range(1):
            ...
            axes[x,y].get_yaxis().set_visible(True)
            axes[x,y].set_xlabel('time (s)')
            axes[x,y].set_ylabel('Frequency(KHz)')
            i += 1
...
def plot_mfccs(mfccs):
    ...
    for x in range(2):
        for y in range(1):
            ...
            axes[x,y].get_yaxis().set_visible(True)
            axes[x,y].set_xlabel('time (s)')
            axes[x,y].set_ylabel('MFCC Coefficients')
            i += 1
...
for c in classes:
    ...
    signal, rate = librosa.load('wavfiles/'+wav_file, sr=200)
...
if len(os.listdir('clean-train_512fs200')) == 0:
    for f in tqdm(df.fname):
        signal,rate = librosa.load('wavfiles/'+f, sr=200)
        mask = envelope(signal, rate, 0.02)
        wavfile.write(filename ='clean-train_512fs200/'+f, rate = rate, data = signal[mask])
